<script>
  // Daten aus der load-Funktion und Form-Responses übernehmen
...
<div class="max-w-3xl mx-auto px-4 py-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <a
              ...
              class="text-sm font-bold text-gray-800 hover:text-pink-500 transition-colors"
            >
              {image.username}
            </a>
          </div>
      ...
      <div class="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-gray-50">
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-rose-50 text-rose-600 font-black text-sm shadow-inner">
            ❤️ {image.votes.toLocaleString()}
            {image.votes === 1 ? 'vote' : 'votes'}
          </span>
  ...
  <div class="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 shadow-sm">
    ...
    <h2 class="text-xl font-black mb-6 text-gray-900 tracking-tight flex items-center gap-2">
      Comments
      <span class="text-sm font-medium text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
        {comments.length}
      </span>
    </h2>
      ...
      {#each comments as comment}
        ...
        <div class="bg-gray-50/70 border border-gray-100/80 rounded-2xl p-4 transition-all hover:bg-gray-50">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-sm font-bold text-gray-800 hover:text-pink-500 cursor-pointer">
              {comment.username}
            </span>
    ...
    {:else}
      ...
      <div class="text-center p-4 bg-pink-50/30 rounded-2xl border border-pink-100/50">
        <p class="text-sm text-gray-600 font-medium">
          Want to engage?
          <a href="/login" class="text-pink-600 font-bold hover:underline transition-all">
            Log in
          </a>
          to favorite or drop a comment.
        </p>
      </div>
